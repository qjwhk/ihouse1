package com.lierda.kesi.ihouse.business;
...
import com.lierda.kesi.ihouse.business.entity.ChannelPTZInfo.Direction;
import com.lierda.kesi.ihouse.business.entity.ChannelPTZInfo.Duration;
import com.lierda.kesi.ihouse.business.entity.ChannelPTZInfo.Operation;
import com.lierda.kesi.ihouse.business.entity.AlarmMessageInfo;
...
import com.lierda.kesi.ihouse.util.OpenApiHelper;
import com.lierda.kesi.ihouse.util.TaskPoolHelper;
import com.lierda.kesi.ihouse.util.TaskPoolHelper.RunnableTask;
...
public class Business {
	...
	private RetObject request(BaseRequest req, int timeOut) {
		...
		BaseResponse t = null;
//		Log.e(tag, req.getBody());
		String a=req.getBody();
		try {
//			Log.e(tag, req.getBody());
...
//				((AccessToken.Response)retObject.resp).data.accessToken).sendToTarget();
			...
			public void run() {
				...
				if (resp != null && resp.data != null
					...
					for (DevicesElement devElement : resp.data.devices) {
						List<ChannelInfo> list = devicesElementToResult(devElement,null);
			...
			public void run() {
				...
				if (resp != null && resp.data != null
					...
					for (ShareDeviceList.ResponseData.DevicesElement devElement : resp.data.devices) {
						List<ChannelInfo> list = devicesElementToResult(null, devElement);
	 ...
	 * @return
	 */
	private List<ChannelInfo> devicesElementToResult(DeviceList.ResponseData.DevicesElement devElement,
                                                     ShareDeviceList.ResponseData.DevicesElement shareDevElement) {

		List<ChannelInfo> channelList = new ArrayList<ChannelInfo>();
		// 设备信息解析
		// Attributes deviceInfo = new Attributes();
		...
		if (devElement != null && devElement.channels != null) {
			for (DeviceList.ResponseData.DevicesElement.ChannelsElement chnlElement : devElement.channels) {
				...
				// 设置设备能力集
				chnlInfo.setAbility(StringToAbility(devElement.ability));
	...
	public void connectWifi(final String BSSID, final String ssid,
		...
		TaskPoolHelper.addTask(new RunnableTask("real") {
			...
			public void run() {
				...
				req.data.token="Ut_cfd8374cc9e643fdbe67559d532ad44f";
